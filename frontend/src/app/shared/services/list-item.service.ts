...
export class ListItemService extends BaseService {
  ...
  getListItemsByListId(listId: number): Observable<ListItemResponseDTO[]> {
    return this.http
      .get<ListItemResponseDTO[]>(`${this.endpoint}/${listId}/items`)
  ...
  addListItem(
    listId: number,
    item: Omit<ListItemRequestDTO, 'listId'>,
  ): Observable<ListItemResponseDTO> {
    ...
    return this.http
      .post<ListItemResponseDTO>(`${this.endpoint}/${listId}/items`, request)
      .pipe(catchError(this.handleError));
  }
  ...
  deleteListItem(listId: number, itemId: number): Observable<void> {
    return this.http
      .delete<void>(`${this.endpoint}/${listId}/items/${itemId}`)
      .pipe(catchError(this.handleError));
  }
}
